...
// Konfiguration
//constexpr Compiler kennt den Wert schon beim Übersetzen.
constexpr std::uint32_t FIELD_SIZE = 100U;
...
//ende überprüfen
std::uint32_t checkEnd(std::uint32_t playerPosition, std::uint32_t enemyPosition, bool playerJump)
{
    if (playerPosition == enemyPosition && playerJump == false)
    {
